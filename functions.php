...
echo '<li role="presentation" class="divider"></li><li role="presentation" class="dropdown-header">'.$type.'</li>';
  ...
  // Display title of each markup samples as a select option
  function listMarkupAsOptions ($type, $fullpage=FALSE) {
    $files = array();
    ...
    foreach ($files as $file):
        ...
        $title = preg_replace("/\-/i", " ", $filename);
        $title = ucwords($title);
        echo $fullpage ? '<li><a href="?v='.$version.'&page='.$filename.'">'.$title.'</a></li>' :
          '<li><a href="'.$indexpage.'#sg-'.$filename.'">'.$title.'</a></li>';
    endforeach;
  }
  ...
  // Display markup view & source
  function showMarkupPage($type, $filename = NULL) {
    $files = array($filename.'.html');
